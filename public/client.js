$(document).ready(function() {
  ...
  $.getJSON('/api/books', function(data) {
    ...
    $.each(data, function(i, val) {
      items.push('<li class="bookItem" id="' + i + '">' + val.title + ' - ' +
        val.commentcount + ' comments</li>');
      return (i !== 14);
    });
    if (items.length >= 15) {
  ...
  $('#display').on('click', 'li.bookItem', function() {
    ...
    $.getJSON('/api/books/' + itemsRaw[this.id]._id, function(data) {
      ...
      comments.push(
        '<br><button class="btn btn-info addComment" id="' + data._id +
        '">Add Comment</button>');
      comments.push(
        '<button class="btn btn-danger deleteBook" id="' + data._id +
        '">Delete Book</button>');
